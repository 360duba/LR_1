...
ATOM MyRegisterClass_SubWindows(HINSTANCE hInstance)
{
    ...
    childwcex.hIcon = LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(IDI_LR1));//参考文献: https://blog.csdn.net/baidu_24190413/article/details/47430551;
    ...
    childwcex.hIconSm = LoadIcon(childwcex.hInstance, MAKEINTRESOURCE(IDI_LR1));
...
void createStatusSheetWindows(HWND mainwindows_hWnd) {
    ...
    lvcolum.iSubItem = 0;
    lvcolum.pszText = CheckLR1Core.toTCHAR(std::string{ " " });
    ListView_InsertColumn(hwndList_StatusSheet, lvcolum.iSubItem, &lvcolum);

    lvcolum.iSubItem++;
    for (auto i = 0; i != 30; i++) {
        lvcolum.pszText = CheckLR1Core.toTCHAR(" ");
        ListView_InsertColumn(hwndList_StatusSheet, lvcolum.iSubItem, &lvcolum);
        lvcolum.iSubItem++;
    }
...
void CreateFirstSetWindow(HWND mainwindows_hWnd) {
    ...
    lvcolum.iSubItem = 0;
    TCHAR orders[] = TEXT("编号"), lefts[] = TEXT("非终结符"), rights[] = TEXT("表达式");

    lvcolum.pszText = orders;
    ListView_InsertColumn(hwndList_FirstSet, lvcolum.iSubItem, &lvcolum);
    lvcolum.iSubItem++;
...
void StartFirstSetWindow(HWND mainwindows_hWnd) {
    ...
    if (GetWindowInfo(hwndList_FirstSet, tmp) == TRUE) {
        ...
        CreateFirstSetWindow(mainwindows_hWnd);
    }
    ChangeSizetoDefault(mainwindows_hWnd);
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        case ID_FILE_LR1CHECK:

            CheckLR1Core.BindInputHWND(main_RichTextBox_VAR);
            //CheckDACore.BindOutputHWND(main_underoutput_RichTextBox_VAR);
            CheckLR1Core.BindOutputHWND(hwndList);
            CheckLR1Core.DoCheck();
            break;
